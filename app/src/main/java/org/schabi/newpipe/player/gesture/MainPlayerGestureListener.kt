...
class MainPlayerGestureListener(
    ...
    private var gestureSeekPosition: Long = 0L
    ...
    override fun onScrollEnd(event: MotionEvent) {
        ...
        if (isSeekingGesture) {
            player.seekTo(gestureSeekPosition)
    ...
    override fun onScroll(
        ...
        // Handle horizontal scrolling for seek gesture
        if (isSeekingGesture || (!isMoving && abs(distanceX) > abs(distanceY) && player.currentState != Player.STATE_COMPLETED)) {
            val duration = player.getExoPlayer()?.duration ?: 0L
            if (duration > 0) {
                if (!isSeekingGesture) {
                    isSeekingGesture = true
                    isMoving = true
                    gestureSeekPosition = player.getExoPlayer()?.currentPosition ?: 0L
                }
                
                // Map swiping full width to 90 seconds seek
                val scrollFraction = -distanceX / binding.root.width.toFloat()
                val seekChange = (scrollFraction * 90000).toLong()
                gestureSeekPosition = (gestureSeekPosition + seekChange).coerceIn(0L, duration)
                
                // Update SeekBar and current time text in real time
                binding.playbackSeekBar.progress = ((gestureSeekPosition.toFloat() / duration) * binding.playbackSeekBar.max).toInt()
                binding.playbackCurrentTime.text = PlayerHelper.getTimeString(gestureSeekPosition)
